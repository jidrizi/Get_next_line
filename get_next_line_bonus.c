...
/*   Updated: 2024/05/22 19:22:27 by jidrizi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (r_line == (void *)1)
		return (ft_bzero(sstring[fd], BUFFER_SIZE), NULL);
	if (sstring[fd][0] == 0 && r_line == NULL)
		return (NULL);
	new_line = ft_strchr(sstring[fd], '\n');
	if (new_line != NULL)
		*new_line = '\0';
	r_line = ft_strjoin(r_line, sstring[fd]);
	if (new_line != NULL)
		r_line = ft_strjoin(r_line, "\n");
	if (r_line == NULL)
		return (NULL);
	if (new_line != NULL && new_line[1] != '\0')
		ft_strlcpy(sstring[fd], new_line + 1, ft_strlen(new_line + 1) + 1);
	else
		ft_bzero(sstring[fd], BUFFER_SIZE + 1);
	return (r_line);
}
